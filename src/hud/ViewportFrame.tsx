...
const CORNERS = [
  ...
  { bottom: 8, left: 8, borderBottom: 2, borderLeft: 2 },
  { bottom: 8, right: 8, borderBottom: 2, borderRight: 2 },
] as const

// The ship's window: vignette, thin frame, and corner brackets drawn over the
// scene. Purely decorative — lets no pointer events through it.
export function ViewportFrame() {
  return (
    <Box sx={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
      <Box sx={{ position: 'absolute', inset: 0, boxShadow: 'inset 0 0 140px 30px rgba(0, 6, 16, 0.9)' }} />
      <Box sx={{ position: 'absolute', inset: '12px', border: `1px solid ${BORDER}`, borderRadius: '14px' }} />
      {CORNERS.map((c, i) => (
        <Box
          key={i}
          sx={{
